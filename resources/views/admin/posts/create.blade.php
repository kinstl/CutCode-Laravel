...
@section('content')
    <div class="container mx-auto px-6 py-8">
        <h3 class="text-gray-700 text-3xl font-medium">
            {{ isset($post) ? 'Редактировать статью ' . $post->id : 'Добавить статью' }}
        </h3>
        ...
        <div class="mt-8">
            <form enctype="multipart/form-data" class="space-y-5 mt-5" method="POST"
                ...
                <input name="title" type="text" value="{{ isset($post) ? $post->title : '' }}"
                    class="w-full h-12 border border-gray-800 @error('title') border-red-500 @enderror rounded px-3"
                    placeholder="Название" />
                @error('title')
                    <p class="text-red-500">{{ $message }}</p>
                @enderror

                <input name="preview" type="text" value="{{ isset($post) ? $post->preview : '' }}"
                    class="w-full h-12 border border-gray-800 @error('preview') border-red-500 @enderror rounded px-3"
                    placeholder="Краткое описание" />
                @error('preview')
                    <p class="text-red-500">{{ $message }}</p>
                @enderror

                <input name="description" type="text" value="{{ isset($post) ? $post->description : '' }}"
                    class="w-full h-12 border border-gray-800 @error('description') border-red-500 @enderror rounded px-3"
